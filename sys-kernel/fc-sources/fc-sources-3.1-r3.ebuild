...
# SPARC64
#
	use sparc64 && epatch "${FILESDIR}"/"${PVR}"/linux-2.6.29-sparc-IOC_TYPECHECK.patch

#
# ARM
#
	use arm && epatch "${FILESDIR}"/"${PVR}"/arm-omap-dt-compat.patch
	use arm && epatch "${FILESDIR}"/"${PVR}"/arm-smsc-support-reading-mac-address-from-device-tree.patch
...
# Patches headed upstream
	...
	epatch "${FILESDIR}"/"${PVR}"/fix-intel-duplicate-backlight.patch
...
# rhbz#605888
	...
	epatch "${FILESDIR}"/"${PVR}"/iwlagn-revert-max-aggregate-size.patch
